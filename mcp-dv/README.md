...
3. Locate the Sign on Success node and place a Get User Consent node immediately after it.
...
2. Set remote environment variables using wrangler
    ...
    | API_IDENTIFIER | ID of the upstream Todo API resource | `https://todo.api.com` |
...
4. **Exchange:** The MCP server exchanges the returned authorization code for an upstream access token.
5. **Binding:** The server binds this upstream token to a new, isolated client session.
...
5. **Functionality:** Upstream API actions (adding/deleting todos) complete successfully.
